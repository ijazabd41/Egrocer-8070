...
async function main() {
  ...
  try {
    ...
    // Fetch a variant ID
    const productsRes = await getJson('/api/product', { limit: 1 }, sessionId);
    ...
    // Add product
    await getJson('/api/order-line/create', { order_id: orderId, product_id: variantId }, sessionId);
    
    // Update order with ONLY carrier_id = 3 (Store Pickup) and see if Odoo applies discount automatically
    console.log("Setting carrier_id = 3 (Store Pickup) without discount params...");
    await getJson(`/api/order/${orderId}/update`, { carrier_id: 3 }, sessionId);
    
    // Fetch order to see if a discount was automatically applied
    let ord = await getJson(`/api/order/${orderId}`, {}, sessionId);
    console.log("Order details after carrier set:", JSON.stringify(ord.data[0], ['id', 'name', 'amount_untaxed', 'amount_tax', 'amount_total', 'amount_discount', 'carrier_id'], 2));
